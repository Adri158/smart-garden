...
export default function AdminDashboard() {
  ...
  async function loadFeedback(page = fbPage, cat = fbCat, search = fbSearch) {
    setFbLoading(true);
    try {
      const params = new URLSearchParams({ page, category: cat, search });
      const res  = await fetch(PHP_BASE + '/admin/feedback?' + params, { credentials: 'include' });
  ...
  async function loadAdmins() {
    try {
      const res  = await fetch(PHP_BASE + '/admin/admins', { credentials: 'include' });
      const data = await res.json();
  ...
  async function submitReply() {
    ...
    try {
      const res  = await fetch(PHP_BASE + `/admin/feedback/${replyTarget.id}/reply`, {
  ...
  async function deleteFeedback(id) {
    ...
    try {
      const res  = await fetch(PHP_BASE + `/admin/feedback/${id}`, {
  ...
  async function addAdmin() {
    setAdAlert({ type: '', msg: '' });
    try {
      const res  = await fetch(PHP_BASE + '/admin/admins', {
        method: 'POST', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteAdmin(id, username) {
    ...
    try {
      const res  = await fetch(PHP_BASE + `/admin/admins/${id}`, {
        method: 'DELETE', credentials: 'include',
      });
      const data = await res.json();
